...
type Server struct {
	finfeedsatv1.UnimplementedMarketFeedServiceServer
	finfeedsatv1.UnimplementedIngestionServiceServer
	finfeedsatv1.UnimplementedOperationsServiceServer
	finfeedsatv1.UnimplementedModelServiceServer
	finfeedsatv1.UnimplementedSemanticServiceServer
...
func (s *Server) GetFeedHealth(context.Context, *finfeedsatv1.GetFeedHealthRequest) (*finfeedsatv1.FeedHealth, error) {
	return toProtoHealth(s.pipeline.FeedHealth()), nil
}

func (s *Server) GetActiveSource(context.Context, *finfeedsatv1.GetActiveSourceRequest) (*finfeedsatv1.ActiveSource, error) {
	health := s.pipeline.FeedHealth()
	return &finfeedsatv1.ActiveSource{
		SourceId: health.SourceID,
		State:    toProtoFeedState(health.State),
	}, nil
}

func (s *Server) StreamBars(request *finfeedsatv1.StreamBarsRequest, stream finfeedsatv1.IngestionService_StreamBarsServer) error {
...
func (s *Server) GetBarWindow(_ context.Context, request *finfeedsatv1.GetBarWindowRequest) (*finfeedsatv1.BarWindow, error) {
	symbol := strings.ToUpper(strings.TrimSpace(request.GetSymbol()))
	if symbol == "" {
		return nil, status.Error(codes.InvalidArgument, "symbol is required")
	}
	bars := s.pipeline.Window(symbol, int(request.GetLimit()))
	out := make([]*finfeedsatv1.Bar, 0, len(bars))
	for _, bar := range bars {
		out = append(out, toProtoBar(bar))
	}
	return &finfeedsatv1.BarWindow{Symbol: symbol, Bars: out}, nil
}

func (s *Server) TriggerGapFill(ctx context.Context, request *finfeedsatv1.TriggerGapFillRequest) (*finfeedsatv1.GapFillResult, error) {
	symbol := strings.ToUpper(strings.TrimSpace(request.GetSymbol()))
	if symbol == "" {
		return nil, status.Error(codes.InvalidArgument, "symbol is required")
	}
	...
	result := &finfeedsatv1.GapFillResult{
		Symbol:           symbol,
...
func (s *Server) GetHealth(context.Context, *finfeedsatv1.GetHealthRequest) (*finfeedsatv1.HealthReport, error) {
	...
	out := &finfeedsatv1.HealthReport{State: toProtoComponent(report.State)}
	for _, component := range report.Components {
		out.Components = append(out.Components, &finfeedsatv1.ComponentHealth{
			Name:   component.Name,
...
func (s *Server) GetReadiness(context.Context, *finfeedsatv1.GetReadinessRequest) (*finfeedsatv1.ReadinessReport, error) {
	ready := s.pipeline.Readiness()
	return &finfeedsatv1.ReadinessReport{
		Ready:   ready.Ready,
...
func toProtoBar(bar domain.Bar) *finfeedsatv1.Bar {
	return &finfeedsatv1.Bar{
		Symbol:              bar.Symbol,
...
func toProtoHealth(health domain.FeedHealth) *finfeedsatv1.FeedHealth {
	var lastMsg int64
	if !health.LastMessage.IsZero() {
		lastMsg = health.LastMessage.UnixMilli()
	}
	return &finfeedsatv1.FeedHealth{
		SourceId:                     health.SourceID,
		State:                        toProtoFeedState(health.State),
...
func toProtoInstrument(value domain.InstrumentType) finfeedsatv1.InstrumentType {
	switch value {
	case domain.InstrumentStock:
		return finfeedsatv1.InstrumentType_INSTRUMENT_TYPE_STOCK
	case domain.InstrumentETF:
		return finfeedsatv1.InstrumentType_INSTRUMENT_TYPE_ETF
	case domain.InstrumentIndex:
		return finfeedsatv1.InstrumentType_INSTRUMENT_TYPE_INDEX
	default:
		return finfeedsatv1.InstrumentType_INSTRUMENT_TYPE_UNSPECIFIED
	}
}

func toProtoQuality(value domain.QualityStatus) finfeedsatv1.QualityStatus {
	switch value {
	case domain.QualityComplete:
		return finfeedsatv1.QualityStatus_QUALITY_STATUS_COMPLETE
	case domain.QualityDegraded:
		return finfeedsatv1.QualityStatus_QUALITY_STATUS_DEGRADED
	case domain.QualityStale:
		return finfeedsatv1.QualityStatus_QUALITY_STATUS_STALE
	case domain.QualityPartial:
		return finfeedsatv1.QualityStatus_QUALITY_STATUS_PARTIAL
	case domain.QualityReconstructed:
		return finfeedsatv1.QualityStatus_QUALITY_STATUS_RECONSTRUCTED
	case domain.QualityInvalid:
		return finfeedsatv1.QualityStatus_QUALITY_STATUS_INVALID
	default:
		return finfeedsatv1.QualityStatus_QUALITY_STATUS_UNSPECIFIED
	}
}

func toProtoFeedState(value domain.FeedState) finfeedsatv1.FeedState {
	switch value {
	case domain.FeedHealthy:
		return finfeedsatv1.FeedState_FEED_STATE_HEALTHY
	case domain.FeedDegraded:
		return finfeedsatv1.FeedState_FEED_STATE_DEGRADED
	case domain.FeedFailed:
		return finfeedsatv1.FeedState_FEED_STATE_FAILED
	case domain.FeedRecovering:
		return finfeedsatv1.FeedState_FEED_STATE_RECOVERING
	default:
		return finfeedsatv1.FeedState_FEED_STATE_UNSPECIFIED
	}
}
...
func toProtoComponent(value domain.ComponentState) finfeedsatv1.ComponentState {
	switch value {
	case domain.ComponentHealthy:
		return finfeedsatv1.ComponentState_COMPONENT_STATE_HEALTHY
	case domain.ComponentDegraded:
		return finfeedsatv1.ComponentState_COMPONENT_STATE_DEGRADED
	case domain.ComponentUnavailable:
		return finfeedsatv1.ComponentState_COMPONENT_STATE_UNAVAILABLE
	default:
		return finfeedsatv1.ComponentState_COMPONENT_STATE_UNSPECIFIED
	}
}
