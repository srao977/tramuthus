# Fin_FeedSat_1 — APTF Volume Engine Full Repository Forensic Investigation and Go Refactorability Assessment

**Title:** Fin_FeedSat_1 — APTF Volume Engine Full Repository Forensic Investigation and Go Refactorability Assessment  
**Date:** 2026-09-05  
**Status:** INVESTIGATION COMPLETE — HUMAN REVIEW REQUIRED  
**Purpose:** Reconstruct the complete validated APTF Volume scientific/runtime path — from raw market volume through numerical preparation, stateful VolumeEngine processing, VolumeEmission, validation, and first downstream convergence — and assess which parts can be faithfully refactored into Go for Fin_FeedSat_1.  
**Scope:** Read-only executable and documentary forensics of the historical APTF repository, plus contextual reading of Fin_FeedSat_1 P-01–P-04 / StageTransition V1.1 and the SADE Go-refactorability investigation. No implementation. No scientific change. No process-model, proto, or StageTransition change.  
**Documentation path:** This repository’s established investigation path is `docs/investigations/` (used by `Fin_FeedSat_1_STAGE_TRANSITION_LIVE_RUN_AUDIT_2026-09-04.md`). This document is stored there.
...
**D01 `update_volume_influence` is a different path.** It consumes raw volume inside Adaptive effective-mass and is already present in Fin_FeedSat_1 P-03. It does not produce `V_N`, `V1`, `V2`, `VolumeEmission`, or cockpit color. SADE deferred the standalone Volume Pipeline; that deferral is not a scientific rejection.

**Go refactorability:** the complete production path appears Go-refactorable. No Volume step has a justified reason to remain in Python. The numerically sensitive step (causal quadratic `lstsq`) is the same method Fin_FeedSat_1 P-04 already implemented in gonum. The policy shell is scalar threshold/hysteresis logic. The missing work is not a Python residue — it is the **upstream feature pipeline** (normalization, derivatives, interval mean) plus an equivalence corpus against frozen 009V/010/014C artifacts.
...
Fin_FeedSat_1 today: P-01 → P-02 → collocated P-03 Adaptive and P-04 Price Engine. `domain.Bar.Volume` is already on the accepted eligible bar. P-03 already applies D01 volume *influence*. There is no standalone Volume process and no Volume StageTransition.
...
| Fin_FeedSat_1 HEAD at investigation start | `07417d9c85799949cd3b173067795905513af173` on `main` |
...
| SADE deferral record | `Fin_FeedSat_1/SADE_GO_REFACTORABILITY_AND_SCALED_RUNTIME_INVESTIGATION_2026-08-27.md` |
| Fin_FeedSat_1 process model (read only) | `docs/design/Fin_FeedSat_1_PROCESS_MODEL_082926.md` |
| Frozen StageTransition (read only) | V1.1 at Fin_FeedSat_1 HEAD |
...
Frozen mature Volume policy (014C), not Fin_FeedSat_1 runtime config:
...
- Fin_FeedSat_1 elastic-interval ideas are **not** retrofitted onto APTF.
- Historical entity name `SPY` appears only as APTF evidence. It is not a proposed Fin_FeedSat_1 hard-code.

## Explicit Exclusions

- No Volume implementation.
- No Fin_FeedSat_1 master-process update and no P-05 numbering decision.
- No proto, StageTransition, Snapshot, Persistence, Mongo, or DNA / Fin_FeedSat_1_transaction work.
...
| | Volume V1/V2 | Price P1/P2 (APTF 009 / Fin_FeedSat_1 P-04) |
|---|---|---|
| Series | `V_N` | close / price |
| Window | **3** | **15** (Fin_FeedSat_1 `causalQuadraticAtIndex`) |
| Fit | `y = a x² + b x + c`, x = minutes − current | same |
| Outputs | V1=`b`, V2=`2a` | p1=`b`, p2=`2a` |
| Library | `np.linalg.lstsq` | Fin_FeedSat_1 `pricing.lstsq` / gonum |

**PROVEN BY EXECUTABLE CODE** APTF `causal_quadratic` vs Fin_FeedSat_1 `internal/pricing/derivatives.go` L5–28.  
...
Fin_FeedSat_1 elastic/nonlinear interval ideas apply to neither without a future design. APTF feature intervals are count-based. APTF color intervals are fixed-cadence.
...
| D01 reference_alpha 0.05, bounds [0,3], `/10.0` | Adaptive | D01 design | **other path** | Yes inside Adaptive | Already Fin_FeedSat_1 Adaptive config |
...
| D01 `VolumeReference` | Adaptive | 1.0 (Fin_FeedSat_1) | EMA | symbol reset | 1 scalar | entity | Fin_FeedSat_1 worker reset | yes | no |
...
### 9.3 What Fin_FeedSat_1 already stores
...
**UNKNOWN:** live Fin_FeedSat_1 `LiveFresh` / gap-fill interaction with a 15-observation Volume window. APTF batch assumed a complete ordered 007 series.
...
| Does Price consume Volume output? | **No** in 014/014B Price engine. `observation.volume` exists on Price `MarketObservation` but PriceEngine does not read it for color (prior forensic). Fin_FeedSat_1 P-04 stores volume in history but derivatives use close | prior APTF Price forensic; Fin_FeedSat_1 `derivatives.go` uses prices |
...
### 12.1 D01 / Fin_FeedSat_1 Adaptive influence (already live in P-03)

APTF `d01_adaptive_parametric_model/src/d01/v02/volume.py` L8–15  
SADE `sade/d01/v02/volume.py` L8–15  
Fin_FeedSat_1 `internal/adaptive/volume.go` L5–11
...
| Fin_FeedSat_1 today | **Implemented in P-03** | **Absent** |
...
| Module / function | Class | Reason | Python deps | Go equivalent already in Fin_FeedSat_1 | Risk | Required equivalence |
|---|---|---|---|---|---|---|
...
| D01 `update_volume_influence` | already G2 in Fin_FeedSat_1 | already ported | math | `adaptive.updateVolumeInfluence` | n/a | existing Adaptive tests |
...
| Operation | Library | Where | Fin_FeedSat_1 equivalent | Add dependency? |
|---|---|---|---|---|
...
gonum is already in Fin_FeedSat_1 `go.mod` (`gonum.org/v1/gonum v0.17.0`). **Do not add libraries for this investigation.**
...
| Reset on symbol reset? | Policy state: yes (mirror 014C session + Fin_FeedSat_1 `ResetSymbol`). Feature rings: **APTF did not reset on session**; whether Fin_FeedSat_1 should reset on `ResetSymbol` is a **future design** question (see risks). |
...
## PART 18 — Current Fin_FeedSat_1 compatibility (no modification)

Already available on the accepted eligible bar / worker:

| Fin_FeedSat_1 surface | Relevance |
|---|---|
...
- decision whether feature windows reset on Fin_FeedSat_1 discontinuity / `ResetSymbol`
...
Smallest scientifically complete Fin_FeedSat_1 Volume capability = **upstream features + mature policy**, not the policy shell alone.
...
| R06 | Color interval requires Δt == 60.0 | `emission_intervals.py` L60 | Fin_FeedSat_1 elastic intervals would split/age differently | Keep intervalizer optional; do not assume Fin_FeedSat_1 elasticity |
...
| R13 | Coupling to one historical entity | 007 `entity_id=SPY`; artifact names `*_SPY_*` | Accidental hard-code in a future port | Forbid named-entity constants in Fin_FeedSat_1 Volume design |
...
| R21 | LiveFresh / discontinuity | Fin_FeedSat_1 P-02 can drop infer; APTF batch had a complete file | Window contamination after gaps | Future design; UNKNOWN in APTF |
...
- Fin_FeedSat_1 `internal/adaptive/volume.go` — already-ported influence
...
- No live Fin_FeedSat_1 Volume behavior exists to compare.
...
- Fin_FeedSat_1 `docs/design/Fin_FeedSat_1_PROCESS_MODEL_082926.md` numbers P-05 as OMS/Risk. That existing number is **not** a Volume process id and was not changed.

---

## Unanswered questions

1. Should a future Fin_FeedSat_1 Volume feature window reset on `ResetSymbol`, session change, or proven gap? APTF features do not reset; APTF policy does. **UNKNOWN for Fin_FeedSat_1.**
...
4. Is color-age (`EmissionIntervalizer`) in the first Fin_FeedSat_1 Volume increment or deferred with 015? Not decided.
...
5. D01/Fin_FeedSat_1 `updateVolumeInfluence` is a **separate Adaptive path** already live in P-03. SADE deferred the standalone Volume Pipeline; that is not a rejection of 009V–014C.
...
| Date | Change |
|---|---|
| 2026-09-05 | Initial forensic investigation document created. Read-only. No APTF, SADE, Fin_FeedSat_1 production, proto, process-model, or StageTransition modification. |