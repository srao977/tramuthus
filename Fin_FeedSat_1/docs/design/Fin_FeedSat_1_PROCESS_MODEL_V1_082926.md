...
**Status:** Process decomposition and service-contract proposal. P-01–P-04 are in-process (P-04 Go PriceEngine/EXPM landed 2 Sep, default `FIN_FEEDSAT_PRICING=off`). P-04V Volume Engine is an approved scientific sibling (design only; implementation not authorized).  
...
**P-04V (design approved):** [Volume Engine](Fin_FeedSat_1_VOLUME_ENGINE_090526.md)
...
Fin_FeedSat_1 v1 is limited to U.S. stocks, ETFs, and published market indices. Indices remain analytics-only and must never become broker orders.
...
**Local Phase 0:** Not started. Not packed into `fin-feedsat-server` by this change.
...
```text
localhost
  fin-feedsat-server            P-01 P-02 P-03 P-04 + Operations
...
**On today:** P-01 (Alpaca), P-02, P-03 (`FIN_FEEDSAT_MODEL=adaptive`), P-04 (`FIN_FEEDSAT_PRICING=expm` opt-in; default `off`). Viewer Price Engine cards and airport boards landed 2 Sep.  
...
```text
cmd/fin-feedsat-server          collocated Go processes (P-01–P-04; P-04V not implemented)
internal/ingestion
internal/adaptive
...
Fin_FeedSat_1-model      P-03 + P-04 + P-04V when implemented (same binary until CPU split)
...
| Phase 0 packing | One Go server (`fin-feedsat-server`); no Python worker |
