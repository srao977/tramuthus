# Fin_FeedSat_1 Realtime Stage State-Transition Publication V1

**Title:** Fin_FeedSat_1 Realtime Stage State-Transition Publication V1  
...
**Authoritative references:** [Process Model](Fin_FeedSat_1_PROCESS_MODEL_082926.md), [P-04 Price Engine](Fin_FeedSat_1_P04_PRICE_ENGINE_090226.md), current `internal/` implementation.  
...
Fin_FeedSat_1 now publishes a compact `Event` when a realtime stage's **meaningful categorical state** changes. Publication is sideways output from P-01–P-04. It is not a new pipeline stage, not a second model mailbox, and not Snapshot or Persistence.
...
Do **not** use Observer / Observation / ObservationEvent. Fin_FeedSat_1 already uses Observation as a scientific input.
...
`stagetransition.Diagnostic` subscribes to the Hub. Default path `./stage_transitions.txt`. Optional override: `Fin_FeedSat_1_STAGE_TRANSITION_LOG`.

`scripts/Start-Fin_FeedSat_1Ingestion.ps1` sets location to the repository root, so the default file is `<repo-root>/stage_transitions.txt`.
...
| Site | Hook |
| :--- | :--- |
| `cmd/fin-feedsat-server/main.go` | `NewHub`, `NewDiagnostic`, `pipeline.SetTransitions`, `modelhost.Options.Transitions`, defer Close |
...
internal/config/config.go          Fin_FeedSat_1_STAGE_TRANSITION_LOG
cmd/fin-feedsat-server/main.go        wiring
```
...
| Config | One optional path env | Fits existing `Fin_FeedSat_1_*` pattern |
