# Fin_FeedSat_1 P-03 — Implementation Increment

**Date:** August 31, 2026  
**Last updated:** September 4, 2026
**Status:** Phases A–E complete in-process (Go adaptive box + host + `ModelService.StreamDecisions`). Default `FIN_FEEDSAT_MODEL=off`. Live IEX DecisionEvents observed 1 Sep. P-04 PriceEngine/EXPM Phases A–I landed 2 Sep (default `FIN_FEEDSAT_PRICING=off`). Paper orders remain out of scope.  
**Design:** [P-03 Adaptive Model Host](Fin_FeedSat_1_P03_ADAPTIVE_MODEL_HOST_083126.md)  
...
Out of scope **for this coding increment:** risk, orders, Go EXPM / F4 / PriceEngine, joining adaptive output to PriceEngine, Python `Predict` worker. Adaptive Pipeline decision UI lives in `Fin_FeedSat_1-dashboard` (landed 1 Sep); this repo does not host frontend.

StageTransition V1.1 is sideways publication after committed DecisionEvents. It does not change D01/D02/D04, DecisionEvent, or the host mailbox. See [Stage Transition Publication](Fin_FeedSat_1_STAGE_TRANSITION_PUBLICATION_V1_2026-09-04.md).

Those pricing items are **P-04**, not a discarded design. Design: [P-04 Price Engine](Fin_FeedSat_1_P04_PRICE_ENGINE_090226.md) · [Implementation](Fin_FeedSat_1_P04_IMPLEMENTATION_090226.md). **Go production destination:** PriceEngine decisions on EXPM trajectories (`time_term == false` only). Offline Gold Nugget oracle is SADE `sade/pricing_pipeline/projection.py::solve_cover_rk45_reference`. Do not port RK45, do not wire it, and do not make Fin_FeedSat_1 depend on the APTF repo.
...
**Present (1 Sep):** `internal/domain/decision.go`, `internal/adaptive/`, `SubscribeModelBars`, `internal/modelhost/`, `FIN_FEEDSAT_MODEL`, `ModelService.StreamDecisions`. Adaptive Pipeline viewer in `Fin_FeedSat_1-dashboard` (own proto copy + `ModelService` client). **P-04 present (2 Sep):** `internal/pricing`, `FIN_FEEDSAT_PRICING`, `ModelService.StreamPriceEvents` — see [P-04 implementation](Fin_FeedSat_1_P04_IMPLEMENTATION_090226.md). **Not present:** `Evaluate` / `ModelInferenceService`.
...
| :--- | :--- | :--- |
| `FIN_FEEDSAT_MODEL` | `off` | `off` or `adaptive`. Unknown values: **fail startup**. |
| `FIN_FEEDSAT_MODEL_DEADLINE` | `200ms` | Reject `<= 0` or unreasonably large (e.g. `> 2s`). |
...
- Deadline: `PrepareStep` on a clone; `Commit` only if finished within `FIN_FEEDSAT_MODEL_DEADLINE` (default 200 ms). Timeout / panic / skip leave committed scientific state unchanged.
...
- Config: unknown `FIN_FEEDSAT_MODEL` or bad deadline **fails startup**. Default remains `off` (ingestion unchanged).
...
```powershell
$env:FIN_FEEDSAT_MODEL = "adaptive"
.\scripts\Start-Fin_FeedSat_1Ingestion.ps1 -Feed iex -Symbols AAPL,MSFT,NVDA
# other terminal:
go run ./cmd/fin-feedsat-ingest-client -operation decisions -symbols AAPL -max-bars 0 -timeout 6h
```
...
| `input/generated/**` | — | **No** | — |
| `pricing_pipeline/**` | `internal/pricing` | **P-04** | See [P-04 implementation](Fin_FeedSat_1_P04_IMPLEMENTATION_090226.md). Port EXPM `solve_cover` / `analytic_affine_trajectory`; do **not** port `solve_cover_rk45_reference`. |
...
Do not start `sdx-server` or `python -m sade run` as part of Fin_FeedSat_1 CI.

## 6. Wiring in `fin-feedsat-server`

When `FIN_FEEDSAT_MODEL=adaptive`:
...
| Live IEX field-level DecisionEvents + latency | Manual; regular hours | Observed 1 Sep (`FIN_FEEDSAT_MODEL=adaptive` + `StreamDecisions`; Adaptive Pipeline in `Fin_FeedSat_1-dashboard`) |
...
Phases A–E are green in-process. Adaptive Pipeline landed in `Fin_FeedSat_1-dashboard` (1 Sep). P-04 PriceEngine/EXPM Phases A–I landed 2 Sep ([design](Fin_FeedSat_1_P04_PRICE_ENGINE_090226.md), [implementation](Fin_FeedSat_1_P04_IMPLEMENTATION_090226.md)). Paper orders remain forbidden. Do not port RK45.
...
| September 1, 2026 | Phase D: `internal/modelhost` keyed workers, `PrepareStep`/`Commit` deadline, infer pause-not-reset, panic isolation, `FIN_FEEDSAT_MODEL` validation, `GetHealth` model component. Default still `off`. Proto deferred to Phase E. |
