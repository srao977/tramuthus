# Fin_FeedSat_1 Current Code Functional Specification

**Date:** August 31, 2026  
**Source repository:** `C:\Users\chino\Fin_FeedSat_1`  
**Consumer repository:** `C:\Users\chino\Fin_FeedSat_1-dashboard`  
**Implementation status:** Increment 1 data ingestion

## Purpose

This document describes the functionality currently implemented in the Fin_FeedSat_1 Go repository. It is intended to support continued development of the separate Fin_FeedSat_1 dashboard by recording the runtime architecture, public gRPC contract, data semantics, health behavior, configuration, and known implementation boundaries that the dashboard can rely on today.
...
The current Go application implements the first Fin_FeedSat_1 increment:
...
The Go module is named `Fin_FeedSat_1`, targets Go 1.25.3, and directly depends on Gorilla WebSocket, gRPC-Go, and Protocol Buffers.

| Area | Location in `C:\Users\chino\Fin_FeedSat_1` | Current responsibility |
| :--- | :--- | :--- |
| Server command | `cmd/fin-feedsat-server/main.go` | Loads configuration, constructs adapters and pipeline, hosts gRPC, and handles process signals. |
| Diagnostic client | `cmd/fin-feedsat-ingest-client/main.go` | Invokes stream, window, gap-fill, source, health, and readiness operations. |
| Public contract | `api/proto/Fin_FeedSat_1/v1/Fin_FeedSat_1.proto` | Defines bar and health messages and the three implemented gRPC services. |
| Generated Go API | `gen/Fin_FeedSat_1/v1` | Generated protobuf messages and gRPC client/server bindings. |
...
flowchart LR
    ...
    NEXT --> UI[Fin_FeedSat_1 dashboard]
```
...
`cmd/fin-feedsat-server/main.go` performs the following steps:
...
| `Fin_FeedSat_1_SOURCE` | `alpaca` | Must be `alpaca` or `csv`. |
| `Fin_FeedSat_1_FEED` | `iex` | `test` selects the Alpaca test WebSocket; every other value selects the IEX WebSocket URL. The value is not otherwise rejected. |
| `Fin_FeedSat_1_SYMBOLS` | `AAPL` | Comma-separated symbols; values are trimmed, uppercased, deduplicated, and limited to 30. |
| `Fin_FeedSat_1_INTERVAL` | `1Min` | Loaded into configuration but not used by the current adapters; emitted and fetched bars are always `1Min`. |
| `Fin_FeedSat_1_CSV_PATH` | `AAPL_1min_firstratedata.csv` | CSV input path when `Fin_FeedSat_1_SOURCE=csv`. |
...
When the feed is `test` and `Fin_FeedSat_1_SYMBOLS` is unset, the configured symbol becomes `FAKEPACA`. CSV mode uses only the first configured symbol. Alpaca mode rejects more than 30 symbols, matching the Basic-plan cap encoded in the application.
...
| `SourceTimestamp` | Exact provider or CSV timestamp text. Fin_FeedSat_1 assigns no additional timing semantics to it. |
...
The API package is `finfeedsat.v1`. All services are hosted without TLS or application authentication in the current server.
...
`cmd/fin-feedsat-ingest-client` connects insecurely to `localhost:50051` by default and supports:
...
finfeedsat.v1.OperationsService
...
- `cmd/fin-feedsat-ingest-client/main.go`
...
Generated files under `gen/Fin_FeedSat_1/v1` reproduce the protobuf contract and are not treated as a separate source of business behavior.
