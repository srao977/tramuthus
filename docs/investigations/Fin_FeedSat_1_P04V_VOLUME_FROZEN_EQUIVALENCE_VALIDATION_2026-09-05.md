# Fin_FeedSat_1 P-04V Volume Frozen APTF Equivalence Validation

**Title:** Fin_FeedSat_1 P-04V Volume Frozen APTF Equivalence Validation  
**Date:** 2026-09-05  
...
Go P-04V **does not reproduce frozen APTF cockpit_color / transition_state**. Production `confirmColor` keeps the last confirmed color in `InterpretationState.Color` while pending (approved Fin_FeedSat_1 Phase D design). Frozen HEAD `spy_volume_engine.observe` writes the **emitted cockpit color** into `VolumePolicyState.color` (AMBER while pending). That single bookkeeping difference is sufficient to diverge later Indicator values.
...
This was not a clean scientific PASS. It was also not an alignment, warm-up, or session-harness failure. Feature/derivative equivalence stood. Indicator equivalence failed until humans authorized changing Fin_FeedSat_1 confirmation state-write to the frozen APTF rule.
...
| Item | Value |
|---|---|
| Fin_FeedSat_1 branch | `main` |
| Fin_FeedSat_1 HEAD | `07417d9c85799949cd3b173067795905513af173` |
...
| Design | `docs/design/Fin_FeedSat_1_VOLUME_ENGINE_090526.md` |
| Implementation | `docs/implementations/Fin_FeedSat_1_P04V_VOLUME_ENGINE_090526.md` |
| Forensic parent | `docs/investigations/Fin_FeedSat_1_APTF_VOLUME_ENGINE_GO_REFACTORABILITY_INVESTIGATION_2026-09-05.md` |
...
- Historical `cockpit_color` is terminology-identical to Fin_FeedSat_1 `VolumeEvent.Indicator`. Frozen CSV columns were not renamed.
...
| Fin_FeedSat_1 branch | `main` |
| Fin_FeedSat_1 HEAD | `07417d9c85799949cd3b173067795905513af173` |
| Pre-existing dirty (preserved, not cleaned) | `docs/design/Fin_FeedSat_1_PROCESS_MODEL_082926.md`; `internal/semantics/loader_test.go`; `internal/semantics/tooling/tooling_test.go`; `internal/server/semantics_test.go` |
...
Full 86MB+ corpus is not vendored. Level 2 discovers `Fin_FeedSat_1_P04V_FROZEN_DIR` or sibling `../APTF/pre08242026_docs` and re-verifies hashes before use.
...
or, if the APTF tree is not beside Fin_FeedSat_1:

```text
set Fin_FeedSat_1_P04V_FROZEN_DIR=C:\Users\chino\APTF\pre08242026_docs
...
- Production Fin_FeedSat_1 does not inherit historical session-reset mechanics.
