# Fin_FeedSat_1 Volume Engine — Scientific and Architectural Design

**Title:** Fin_FeedSat_1 Volume Engine — Scientific and Architectural Design  
**Date:** 2026-09-06  
...
**Purpose:** Define P-04V Volume Engine: validated Volume mathematics, Fin_FeedSat_1 realtime consume/state/emit architecture, inputs, outputs, bounded per-entity state, timing, invariants, and boundaries.  
**Scope:** Scientific and architectural design. Mathematics in this document are frozen. Implementation details, A–G chronology, and host/RPC wiring are recorded in [P-04V implementation record](../implementations/Fin_FeedSat_1_P04V_VOLUME_ENGINE_IMPLEMENTATION_090526.md). This pass does not change Go, proto, StageTransition, or the Process Model file.  
**Parents:** [Process Model](Fin_FeedSat_1_PROCESS_MODEL_V1_082926.md) (filename as of this reconciliation; **contents not edited**), [P-03 Adaptive Model Host](Fin_FeedSat_1_P03_ADAPTIVE_MODEL_HOST_083126.md), [P-04 Price Engine](Fin_FeedSat_1_P04_PRICE_ENGINE_090226.md), [Stage Transition Publication V1.1](Fin_FeedSat_1_STAGE_TRANSITION_PUBLICATION_V1_2026-09-04.md)  
**Implementation record:** [Fin_FeedSat_1_P04V_VOLUME_ENGINE_IMPLEMENTATION_090526.md](../implementations/Fin_FeedSat_1_P04V_VOLUME_ENGINE_IMPLEMENTATION_090526.md)  
**Forensic authority:** [APTF Volume Engine Go Refactorability Investigation, 2026-09-05](../investigations/Fin_FeedSat_1_APTF_VOLUME_ENGINE_GO_REFACTORABILITY_INVESTIGATION_2026-09-05.md)  
**Historical executable authority:** APTF commit `ae0dacb2e02c5b80c82f6662d1a3c6863f4b989a`  
**Fin_FeedSat_1 baseline:** `07417d9c85799949cd3b173067795905513af173` (`Fin_FeedSat_1-stage-transition-v1.1-validated-2026-09-04`)
...
THE VALIDATED APTF VOLUME MATHEMATICS
        implemented inside
THE EXISTING Fin_FeedSat_1 REALTIME
CONSUME → STATE → PROCESS → EMIT
PIPELINE MODEL
```

P-04V is **one Volume Engine**. It owns `VolumeState` and performs both scientific responsibilities inside that engine. There is no separate Fin_FeedSat_1 Volume Policy component, service, layer, or process.
...
The historical APTF class named `VolumeEngine` implemented only interpretation. Fin_FeedSat_1 P-04V includes both responsibilities.

P-03 already uses raw `Bar.Volume` through D01 `updateVolumeInfluence`. That Adaptive path remains and is not P-04V.

P-04V follows the established Fin_FeedSat_1 realtime model used by Adaptive and Price: consume the existing accepted eligible Bar, own bounded per-entity scientific state, prepare/commit without partial mutation, emit a first-class Volume outcome. It does not create another market subscription.
...
There is **no** `Fin_FeedSat_1_VOLUME` flag. Volume is present whenever Adaptive Host exists (`FIN_FEEDSAT_MODEL=adaptive`). Price remains independently `FIN_FEEDSAT_PRICING`.

## Governing principle

APTF historical artifacts define the Volume **scientific transformation** and frozen **migration-equivalence** targets. They do **not** automatically define Fin_FeedSat_1 runtime lifecycle, storage, session, buffering, state-management, or batch-processing semantics.

Where historical APTF runtime/batch mechanics differ from the established Fin_FeedSat_1 realtime architecture, **preserve the validated Volume mathematics** and **use the established Fin_FeedSat_1 realtime architecture** unless the Volume mathematics themselves demonstrably require otherwise.
...
| **Fin_FeedSat_1 REALTIME BEHAVIOR** | How P-04V operates in the live pipeline |

The reader must not infer the category.

## Fin_FeedSat_1 terminology

**Canonical Fin_FeedSat_1:** P-04V Volume Engine · `VolumeState` · Volume Feature State · Volume Interpretation State · Volume Mathematics · Volume Output · **Indicator** (confirmation-controlled activity category) · `raw_color` (pre-confirmation activity band)
...
Historical names remain where forensic traceability or equivalence testing requires them. They are not Fin_FeedSat_1 architectural ontology. P-04V does not introduce a Volume Policy Service, layer, or process.
...
### Fin_FeedSat_1 realtime architecture
...
**Fin_FeedSat_1 REALTIME BEHAVIOR:** The incoming Bar is immutable. P-04V scientific state is private to P-04V. Candidate calculations must not partially mutate committed scientific state. A failed or non-actionable computation must not leave partially advanced state. Implemented prepare/commit is `volume.Engine.PrepareStep` / `Commit`. Volume commit is independent of Adaptive+Price `commitA && commitP`. API names and host order are in the [implementation record](../implementations/Fin_FeedSat_1_P04V_VOLUME_ENGINE_IMPLEMENTATION_090526.md).
...
**Fin_FeedSat_1 REALTIME BEHAVIOR:** the same accepted eligible canonical `domain.Bar` already supplied to P-03 and P-04. No second market subscription. No second `SubscribeModelBars` mailbox.
...
| `MarketSnapshotID` | Existing Fin_FeedSat_1 correlation identity |
...
First-class Volume outcome. Canonical wire names are in `api/proto/Fin_FeedSat_1/v1/Fin_FeedSat_1.proto` (`VolumeEvent` / `VolumeEmission`). Field numbers are implemented; this table is scientific meaning, not a proto redesign.
...
`V_INTERVAL_B10_C2` is historical/frozen scientific identity. It is **not** a Fin_FeedSat_1 Volume Policy Service, layer, or process.

**Canonical Fin_FeedSat_1 terms:** P-04V Volume Engine, `VolumeState`, Volume Feature State, Volume Interpretation State, Volume Mathematics, Volume Output, Indicator.
...
- APTF batch/session/replay mechanics do not automatically become Fin_FeedSat_1 runtime.
- Observation-count windows are not elapsed-minute windows.
- Fin_FeedSat_1 elastic/nonlinear interval concepts are not retrofitted onto this science.
...
- DNA / Fin_FeedSat_1_transaction, Dynamic Meaning Matrix, Decision Engine / Forum
...
- A `Fin_FeedSat_1_VOLUME` operator flag (not implemented; Volume follows Adaptive Host)
...
- Fin_FeedSat_1 P-04 Price uses validated analytic EXPM, not RK45.
- Fin_FeedSat_1 P-04V remains discrete `G_V`.
...
Historical APTF frozen interpretation configuration: `V_INTERVAL_B10_C2` (`V_EMISSION_V0_1` freeze label). This is scientific identity, not a Fin_FeedSat_1 architectural policy layer.
...
**APTF FORENSIC BEHAVIOR:** the 014C batch caller reset historical `VolumePolicyState` at `date:session`. The engine itself has no calendar. That caller mechanic is **not** Fin_FeedSat_1 runtime. P-04V V1 has **no** independent interpretation-state reset. See [Realtime lifecycle](#7-realtime-lifecycle).
...
### 7.1 APTF forensic behavior (not Fin_FeedSat_1 runtime)
...
The mathematics themselves contain **no calendar/session model**. P-04V does **not** inherit `date:session` reset. This design does **not** invent a Fin_FeedSat_1 date:session replacement.

### 7.2 Bounded realtime VolumeState

**Fin_FeedSat_1 REALTIME BEHAVIOR.** Conceptual architecture, not frozen Go structs:
...
### 7.3 Fin_FeedSat_1 lifecycle events

| Event | Fin_FeedSat_1 REALTIME BEHAVIOR |
|---|---|
...
| Infer restored | Continue existing state unless Fin_FeedSat_1 has separately declared an explicit scientific reset/discontinuity. |
...
| Market / session boundary | Not an automatic P-04V reset. Any later explicit lifecycle event must be justified by Fin_FeedSat_1 realtime semantics, not APTF batch caller behavior. |
...
| `IntervalStart` | **Fin_FeedSat_1 REALTIME BEHAVIOR:** established accepted-observation time basis, same as P-04 minutes |
...
**Fin_FeedSat_1 REALTIME BEHAVIOR:** readiness arises from available scientific state, not a dataframe index and not `if observation_number == 29`.
...
| Situation | APTF FORENSIC BEHAVIOR | Fin_FeedSat_1 REALTIME BEHAVIOR |
|---|---|---|
...
| Fin_FeedSat_1 today | Implemented | Implemented (science + Phase G host join) |
