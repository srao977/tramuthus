# Fin_FeedSat_1 Process Model

**Version:** V2  
**Date:** September 6, 2026  
**Derived from:** [Process Model V1](Fin_FeedSat_1_PROCESS_MODEL_V1_082926.md) (29 Aug 2026; last V1 update 5 Sep 2026)  
**Status:** Process decomposition and service-contract proposal. P-01–P-04 are in-process (P-04 Go PriceEngine/EXPM landed 2 Sep, default `FIN_FEEDSAT_PRICING=off`). P-04V Volume Engine is an implemented sibling model-processing process, validated through ModelHost Phase G (6 Sep 2026).  
**Parent Architecture:** [Fin_FeedSat_1 System Specification](Fin_FeedSat_1_hi-level_design_082826.md)  
**Derived Artifact Specification:** [E2E Fin_FeedSat_1 Artifacts](E2E_Fin_FeedSat_1_ARTIFACTS.md)  
**Open Design Gaps:** [Fin_FeedSat_1 Decision Integrity and Design Gap Analysis](Fin_FeedSat_1_DECISION_INTEGRITY_GAP_ANALYSIS_082826.md)

**Increment 1 (P-01 / P-02):** [Fin_FeedSat_1 Ingestion Increment 1](Fin_FeedSat_1_INGESTION_INCREMENT_1_083026.md)  
**Increment 1 continuation (P-02 quality):** [P-02 Data Quality](Fin_FeedSat_1_INGESTION_P02_DATA_QUALITY_083126.md)  
**P-03 (landed):** [Adaptive Model Host](Fin_FeedSat_1_P03_ADAPTIVE_MODEL_HOST_083126.md) · [Implementation](Fin_FeedSat_1_P03_IMPLEMENTATION_083126.md)  
**P-04 (landed, default off):** [Price Engine](Fin_FeedSat_1_P04_PRICE_ENGINE_090226.md) · [Implementation](Fin_FeedSat_1_P04_IMPLEMENTATION_090226.md)  
**P-04V (implemented / Phase G):** [Volume Engine](Fin_FeedSat_1_VOLUME_ENGINE_090526.md) · [Implementation](../implementations/Fin_FeedSat_1_P04V_VOLUME_ENGINE_IMPLEMENTATION_090526.md)

Fin_FeedSat_1 Process Model V2 preserves the process identities, responsibilities, relationships, and implementation-reference structure of V1. V2’s primary architectural change is reconciliation of **P-04V Volume Engine** from design-approved status to its validated Go / protobuf / ModelHost Phase G implementation. Other process IDs and definitions remain unchanged except where minimal relationship text is required to represent P-04V accurately.

## 1. Purpose and Authority

This document translates the parent architecture diagram into a **process model**: named runtime units, their inputs and outputs, failure domains, scale axes, and gRPC contracts. It exists so Fin_FeedSat_1 can define microservices and `Fin_FeedSat_1.proto` from process boundaries rather than from package folders.
...
| Concept | Role |
| :--- | :--- |
| **Entity** | The distinct thing for which Fin_FeedSat_1 maintains causal model processing and process state. |
...
| :--- | :--- | :--- | :--- |
| **Venue: Alpaca Paper** | Alpaca’s paper brokerage API. Fin_FeedSat_1 treats it as the configured broker. | P-06 | **On.** This is the execution venue for local testing. |
| **Internal paper engine** | Fin_FeedSat_1’s L2 fill simulator for live-versus-modeled comparison. | P-09 | **Off** unless benchmark mode is `SAMPLED` or `FULL`. |
...
**Local Phase 0:** Collocated in `fin-feedsat-server`. `FIN_FEEDSAT_MODEL=off` by default; `adaptive` enables the host. Live IEX DecisionEvents observed 1 Sep.
...
**Local Phase 0:** Collocated in `fin-feedsat-server`. `FIN_FEEDSAT_PRICING=off` by default; `expm` requires `FIN_FEEDSAT_MODEL=adaptive`. Phases A–I landed 2 Sep. Continuity is causal observation order: a skipped provider minute is a valid irregular interval, not `INPUT_GAP`. `STATE_DISCONTINUOUS` is reserved for proven loss (overflow / panic / proven missing eligible). `Host.ResetSymbol` reinitializes one symbol; no operator reset RPC yet.

**Oracle:** SADE `solve_cover_rk45_reference` stays outside Fin_FeedSat_1. Go production is EXPM only (`gonum` v0.17.0 `Dense.Exp`).

### 6.4V P-04V Volume Engine

**Status:** Implemented and validated through ModelHost Phase G (6 Sep 2026). Detailed scientific definitions, invariants, lifecycle, maturation, frozen mathematics, and APTF equivalence authority live in [Volume Engine](Fin_FeedSat_1_VOLUME_ENGINE_090526.md). Coding chronology lives in [P-04V implementation](../implementations/Fin_FeedSat_1_P04V_VOLUME_ENGINE_IMPLEMENTATION_090526.md). This catalog does not duplicate those formulas.
...
**Does not own:** Adaptive influence (P-03 D01 `updateVolumeInfluence` remains P-03), PriceEngine mathematics, risk, tradability, broker semantics, Snapshot, Persistence, or a separate Volume Policy process. There is no Fin_FeedSat_1 Volume Policy service, layer, or process.
...
**Local Phase 0:** Collocated in `fin-feedsat-server`. Volume is present whenever Adaptive Host exists (`FIN_FEEDSAT_MODEL=adaptive`). There is **no** `Fin_FeedSat_1_VOLUME` flag. Price remains independently `FIN_FEEDSAT_PRICING`. `Host.ResetSymbol` rebuilds Volume (and Adaptive/Price); there is no automatic session, source, infer, or elapsed-gap Volume reset.
...
SADE Unit Run 001 (adaptive) and Pricing Unit Run 001 are the numerical authorities. Live trading is only valid if Fin_FeedSat_1 Go engines reproduce those frozen sequences, not a Python sidecar.
...
Package: `finfeedsat.v1`. Organize the file by area: common types, market data, model, risk, execution, ledger, benchmark, operations, then service definitions. Do not split the file until review or generation friction is demonstrated.
...
```text
localhost
  fin-feedsat-server            P-01 P-02 P-03 P-04 P-04V + Operations
  ...
  Fin_FeedSat_1-dashboard         C-01 observe + Adaptive Pipeline (not required path)
```

**On today:** P-01 (Alpaca), P-02, P-03 (`FIN_FEEDSAT_MODEL=adaptive`), P-04 (`FIN_FEEDSAT_PRICING=expm` opt-in; default `off`), P-04V (no `Fin_FeedSat_1_VOLUME`; on whenever Adaptive Host exists). Viewer Price Engine cards and airport boards landed 2 Sep.  
...
```text
cmd/fin-feedsat-server          collocated Go processes (P-01–P-04 and P-04V)
internal/ingestion
internal/adaptive
...
```text
Fin_FeedSat_1-ingest     P-01 P-02
Fin_FeedSat_1-model      P-03 + P-04 + P-04V (same binary until CPU split)
Fin_FeedSat_1-risk       P-05
Fin_FeedSat_1-exec       P-06
Fin_FeedSat_1-ledger     P-08
NATS JetStream      P-07
```
...
| :--- | :--- | :--- | :--- | :--- |
| S0 | **Partial** | `Fin_FeedSat_1.proto` increment-1 services generated in Go. Risk, execution, ledger, benchmark, and Python stubs are not in the file yet. | contracts | Ingestion/ops RPCs compile. Full-file golden round-trip still open. |
| S1 | **Partial** | Alpaca IEX/test WebSocket, REST historical client, CSV replay, thin reconnect, bar window, `StreamBars`. Full circuit breaker is **not completed** and is **deferred**. See [Increment 1](Fin_FeedSat_1_INGESTION_INCREMENT_1_083026.md). | P-01, P-02 | CSV and Alpaca test-feed bars received 2026-08-30. Near-term exit is IEX RTH. Failover, live gap-fill proof, and DI-01 qualification are later. |
...
| Increment 1 ingestion implementation and Alpaca/CSV evidence | [Ingestion Increment 1](Fin_FeedSat_1_INGESTION_INCREMENT_1_083026.md) |
| P-03 adaptive host | [P-03 design](Fin_FeedSat_1_P03_ADAPTIVE_MODEL_HOST_083126.md) |
| P-04 PriceEngine / EXPM | [P-04 design](Fin_FeedSat_1_P04_PRICE_ENGINE_090226.md) |
| P-04V Volume Engine science, invariants, lifecycle, frozen mathematics, APTF equivalence | [Volume Engine](Fin_FeedSat_1_VOLUME_ENGINE_090526.md) |
| P-04V implementation record (A–G, proto, Phase G host) | [P-04V implementation](../implementations/Fin_FeedSat_1_P04V_VOLUME_ENGINE_IMPLEMENTATION_090526.md) |
...
| Proto layout | Still one `finfeedsat.v1` file; services listed in §9 |
| Phase 0 packing | One Go server (`fin-feedsat-server`); no Python worker. P-04V collocated when Adaptive Host exists; no `Fin_FeedSat_1_VOLUME`. |
...
## 17. Fin_FeedSat_1 Terminology and Systems Dictionary

This dictionary defines the canonical meaning of terms used by the Fin_FeedSat_1 Process Model. Where a term has a broader meaning in software, control systems, quantitative finance, or process engineering, the definition below states the meaning intended within Fin_FeedSat_1.
...
Current model-processing sequence for P-03 / P-04 / P-04V (not a universal state machine for every Fin_FeedSat_1 process):
...
| Term | Canonical Fin_FeedSat_1 Meaning | Important Distinction / Not the Same As |
| :--- | :--- | :--- |
| **Entity** | The distinct thing for which Fin_FeedSat_1 maintains causal model processing and Process State. In Fin_FeedSat_1 V1, an Entity may represent a supported instrument class: a U.S. stock, an ETF, or a published market index. Indices remain analytics-only Entities and must never become broker orders. | **Not** Symbol. **Not** Instrument as a market-domain object. **Not** the Entity-Key Worker. A Symbol may identify or help identify an Entity; the concepts are not identical. Entity is not defined as “stock.” |
...
| **Symbol** | The market/instrument identifier carried in current market-data contracts and used by the current implementation for routing and grouping. For Fin_FeedSat_1 V1 it is currently sufficient for much of the runtime routing of stocks, ETFs, and indices. | **Not** synonymous with Entity. **Not** the general architectural definition of Entity Key. Do not broaden V1 scope to FX, futures, crypto, or options. |
| **Observation** | Information accepted by Fin_FeedSat_1 as describing an Entity at a particular causal position or interval. Fin_FeedSat_1 already uses observation as a scientific/data concept. This Process Model does not introduce “Observer” as a runtime component. | **Not** synonymous with a raw provider payload. **Not** necessarily a Bar, except where the context specifically refers to the Bar representation used on the current model-processing path. Historical APTF “observer” terminology is provenance only. |
...
| **Bar** | The canonical interval-based market-data representation currently used to carry eligible Observations through the Fin_FeedSat_1 model-processing path. It contains entity/instrument identity, interval/time information, OHLCV, and provenance/quality information according to the current contract. A Bar is data. | **Not** an Entity. **Not** Process State. **Not** a worker. **Not** a Decision. **Not** a Process Output merely because it exists in P-02. |
...
| **Process State** | The authoritative retained information owned by a Process for an Entity and used when processing subsequent Observations. Examples include Adaptive state, Price state, and `VolumeState`. “State” is context-sensitive in Fin_FeedSat_1 documentation and should normally be qualified. | **Not** a Bar. **Not** a database record. **Not** Snapshot. **Not** Persistence. **Not** a StageTransitionEvent. **Not** Process Output. **Not** Stage State. |
...
| **Stage** | A named StageTransition identity (`StageID`) under the existing StageTransition V1.1 contract. Stages are sideways/publication identities, not additional pipeline processes. | **Not** a Fin_FeedSat_1 Process ID. **Not** a new P-number. Do not treat Stage as a substitute for Process State. |
...
| **Causal Order** | The required ordering of Observations for one Entity such that Process State evolves according to the actual accepted/published observation sequence. Fin_FeedSat_1 permits irregular or nonlinear elapsed intervals. | **Not** “every adjacent wall-clock minute.” **Not** wall-clock adjacency. **Not** interval duration. A skipped provider minute may still be a valid next Observation. |
...
| **Service** | A contract surface exposed across a process boundary, currently commonly gRPC in Fin_FeedSat_1. | **Not** necessarily a Process. A Process may expose a Service. Multiple processes may initially be collocated in one binary. |
| **Host** | A runtime role/component term. In the current Fin_FeedSat_1 implementation, the Host receives model-published Bars, performs common gating/routing, associates Observations with the appropriate Entity-Key Worker, and coordinates collocated model-processing execution. P-03 Adaptive Model Host is the named Fin_FeedSat_1 Process that currently owns this Host role. | Host role/component is **not** the generic definition of Process. P-03 Adaptive Model Host **is** a named Fin_FeedSat_1 Process. The architecture does not require the current Host implementation mechanism forever. Do not rename P-03. |
...
| **Engine** | An implementation/component term for deterministic model mathematics or orchestration where the established Fin_FeedSat_1 component uses that name (Adaptive Engine, Price Engine, Volume Engine, `volume.Engine`). A named Engine may participate within a Process. | **Not** an unexplained generic architectural category. Do not introduce “Scientific Engine,” “Engine Plane,” or “Engine Layer.” Engine is not Process in the generic sense. |
...
| **Effective Time** | A time semantic used by some Fin_FeedSat_1 contracts to express when a value is effective. P-04V has no independent Volume EffectiveTime. | **Not** Interval Start. **Not** Source Timestamp. Do not invent a new Volume time base. |
...
| **Aperture** | Related operational/session-boundary term used in other Fin_FeedSat_1 documents around runtime observation and persistence scope. Not expanded by this Process Model. | **Not** the realtime processing master. **Not** defined here as a new architectural layer. |
...
| September 4, 2026 | 0.7 | Sideways StageTransition publication V1.1 (`internal/stagetransition`). P-01–P-04 publish only on meaningful StageState change. Bar-driven P-03/P-04 events carry a value copy of the accepted `domain.Bar`. Not a new pipeline stage. Snapshot/Persistence not implemented. See [Stage Transition Publication](Fin_FeedSat_1_STAGE_TRANSITION_PUBLICATION_V1_2026-09-04.md). |
...
| September 2, 2026 | 0.5 | P-04 Phases A–I landed: `internal/pricing`, host join, `StreamPriceEvents`, dashboard Price Engine cards/boards. Default still `FIN_FEEDSAT_PRICING=off`. Live IEX `INPUT_GAP` documented as missing adjacent minute, not end-of-data. |
