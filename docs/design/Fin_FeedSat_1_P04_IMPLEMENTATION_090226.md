# Fin_FeedSat_1 P-04 — Implementation Increment

**Date:** September 2, 2026  
**Status:** Governing summary accepted 2 Sep. Phases A–I landed (2 Sep). Viewer Price Engine card and airport boards included.  
**Parents:** [P-03 Implementation](Fin_FeedSat_1_P03_IMPLEMENTATION_083126.md), [Process Model](Fin_FeedSat_1_PROCESS_MODEL_082926.md)  
...
Out of scope **for this coding increment:** risk, orders, Python `Predict`, `ModelInferenceService`, RK45 Go port, joining BUY/SELL to color into an order. Viewer Price Engine cards and airport boards landed 2 Sep in `Fin_FeedSat_1-dashboard`.

**Go production destination:** PriceEngine on EXPM trajectories (`time_term == false` only). Offline Gold Nugget (not in this repo, not in CI): SADE `projection.py::solve_cover_rk45_reference`. Fin_FeedSat_1 CI compares to checked-in SADE **EXPM** artifacts.
...
internal/modelhost/          (Phase H — landed 2 Sep)
  ...
  GetHealth pricing component; FIN_FEEDSAT_PRICING validation
  SubscribePriceEvents fan-out (Phase I)

api/proto/.../Fin_FeedSat_1.proto (Phase I)
  ModelService.StreamPriceEvents + PriceEvent / PriceEmission / PricingSkip
```

Do **not** create `internal/sade`. Do **not** add `cmd/Fin_FeedSat_1-model-worker`. `gonum.org/v1/gonum v0.17.0` is declared in `go.mod`.

Suggested flags (startup env):

| Variable | Default | Meaning |
| :--- | :--- | :--- |
| `FIN_FEEDSAT_PRICING` | `off` | `off` or `expm`. Unknown: **fail startup**. |
| `FIN_FEEDSAT_MODEL` | `off` | Must be `adaptive` if pricing is `expm`. |
...
- One transactional pair: `PrepareStep` adaptive + pricing, then commit **both** or **neither**. Shared `FIN_FEEDSAT_MODEL_DEADLINE`. Injected P-04 failure after P-03 prepare rolls back both; replay of the same bar matches a clean run.
...
- Default `off`. `expm` without `adaptive` fails startup. Unknown `FIN_FEEDSAT_PRICING` fails startup.

**Exit tests:** overflow/gap pause pricing; INITIALIZING bars still price-warmup; reconstructed/backfill bars never price; rollback/replay; `FIN_FEEDSAT_PRICING` unknown fails startup. **Done (2 Sep).**
...
Do not run SADE as part of Fin_FeedSat_1 CI.

## 6. Wiring in `fin-feedsat-server`

When `FIN_FEEDSAT_MODEL=adaptive` and `FIN_FEEDSAT_PRICING=expm`:
...
P-03 `StreamDecisions` unchanged. `StreamPriceEvents` is the northbound PriceEvent fan-out. After a Fin_FeedSat_1 proto change, copy `Fin_FeedSat_1.proto` into `Fin_FeedSat_1-dashboard` and regenerate; restart Next.js so `/api/prices` loads.
...
| `time_term` reject; unknown `FIN_FEEDSAT_PRICING` | A / H | Done (2 Sep) |
...
| September 2, 2026 | Phase H: same keyed worker, `FIN_FEEDSAT_PRICING=off|expm`, transactional prepare/commit, rollback/replay, `GetHealth` pricing component. No proto / dashboard. |
