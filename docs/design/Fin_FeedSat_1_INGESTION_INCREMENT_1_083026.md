# Fin_FeedSat_1 Increment 1 — Data Ingestion

**Date:** August 30, 2026  
**Status:** Implemented and locally validated (CSV + Alpaca test feed). Live IEX RTH and P-02 quality gating are recorded in [P-02 Data Quality (August 31)](Fin_FeedSat_1_INGESTION_P02_DATA_QUALITY_083126.md). REST gap-fill against a live disconnect is still open.  
...
Provider-supplied 1-minute bars (`T=b`) are the increment-1 live input. Local tick aggregation is deferred (DI-01 remains open). Bars still carry Fin_FeedSat_1 quality and receipt metadata.
...
Collocated in `cmd/fin-feedsat-server`. Domain packages do not import generated proto. The gRPC adapter in `internal/server` maps at the edge, same rule as SDX.

## 5. Contracts

`api/proto/Fin_FeedSat_1/v1/Fin_FeedSat_1.proto` (`finfeedsat.v1`) defines only the increment-1 services. Later increments append to this file.
...
`source_timestamp` is forwarded verbatim. Fin_FeedSat_1 does not derive session or model time from it. `receipt_unix_ms` is local receive time.
...
- IEX validation during regular hours — **done 2026-08-31**; see [P-02 Data Quality](Fin_FeedSat_1_INGESTION_P02_DATA_QUALITY_083126.md).
...
| August 31, 2026 | Pointed IEX RTH and P-02 quality work at `Fin_FeedSat_1_INGESTION_P02_DATA_QUALITY_083126.md`. |
