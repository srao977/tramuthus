# Fin_FeedSat_1 Semantic Contract V1

**Date:** September 2, 2026  
**Last updated:** September 2, 2026  
**Status:** INITIAL_CANONICAL_BASELINE (cleanup closure)  
**Contract version:** `1.0`  
**JSON:** `internal/semantics/data/finfeedsat_semantics_v1.json`  
**Parents:** [Process Model](Fin_FeedSat_1_PROCESS_MODEL_082926.md), [P-03 Implementation](Fin_FeedSat_1_P03_IMPLEMENTATION_083126.md), [P-04 Implementation](Fin_FeedSat_1_P04_IMPLEMENTATION_090226.md)

## 1. Why it exists

Fin_FeedSat_1 already exposes scientific states such as HOLD, scientific SHORT, HOLDING, ON TIME, DELAYED, UP ACCELERATING, HIGH, LOW, DOMAIN EXIT, and INITIALIZING. Those meanings lived in Go source, proto, tests, and dashboard aliases.
...
Go executable vocabulary
        |
        v
finfeedsat_semantics_v1.json     (source of truth)
        |
        v
...
**Published canonical contract:** `internal/semantics/data/finfeedsat_semantics_v1.json`  
...
Commands (`cmd/fin-feedsat-semantics`):

```text
go run ./cmd/fin-feedsat-semantics validate
go run ./cmd/fin-feedsat-semantics audit
go run ./cmd/fin-feedsat-semantics build
go run ./cmd/fin-feedsat-semantics build --check
```
...
2. `go run ./cmd/fin-feedsat-semantics audit` — expect MISSING if the token is new.
3. Add a curated `semantics.Term` to `internal/semantics/catalog/v1.go`. Do not invent a meaning the code does not support.
4. `go run ./cmd/fin-feedsat-semantics build`
5. `go run ./cmd/fin-feedsat-semantics validate`
...
cmd/fin-feedsat-semantics/main.go
internal/semantics/data/finfeedsat_semantics_v1.json
internal/semantics/{model,loader,validator,embed}.go
api/proto/Fin_FeedSat_1/v1/Fin_FeedSat_1.proto          SemanticService
internal/server/semantics.go
Fin_FeedSat_1-dashboard/src/lib/semantics.ts
Fin_FeedSat_1-dashboard/src/app/semantic-help.tsx
Fin_FeedSat_1-dashboard/src/app/semantics/page.tsx
```
...
| September 2, 2026 | Permanent Go catalog + `fin-feedsat-semantics` validate/audit/build. JSON key order normalized for deterministic encode. Meanings unchanged. Version remains 1.0. |
