# Fin_FeedSat_1 P-04V Volume Engine — Implementation Design and Implementation Record

**Title:** Fin_FeedSat_1 P-04V Volume Engine — Implementation Design and Implementation Record  
**Date:** 2026-09-06  
...
**Parents:** [Volume Engine scientific/architectural design](../design/Fin_FeedSat_1_VOLUME_ENGINE_090526.md), [Process Model](../design/Fin_FeedSat_1_PROCESS_MODEL_V1_082926.md) (filename as of this reconciliation; **contents not edited**), [APTF Volume forensic investigation](../investigations/Fin_FeedSat_1_APTF_VOLUME_ENGINE_GO_REFACTORABILITY_INVESTIGATION_2026-09-05.md)  
**Implementation precedents:** [P-03 Adaptive Model Host](../design/Fin_FeedSat_1_P03_ADAPTIVE_MODEL_HOST_083126.md), [P-03 Implementation](../design/Fin_FeedSat_1_P03_IMPLEMENTATION_083126.md), [P-04 Price Engine](../design/Fin_FeedSat_1_P04_PRICE_ENGINE_090226.md), [P-04 Implementation](../design/Fin_FeedSat_1_P04_IMPLEMENTATION_090226.md)  
**Filename:** `docs/implementations/Fin_FeedSat_1_P04V_VOLUME_ENGINE_IMPLEMENTATION_090526.md` (renamed 2026-09-06 from `Fin_FeedSat_1_P04V_VOLUME_ENGINE_090526.md` so it is not confused with the scientific design document).  
**Fin_FeedSat_1 baseline inspected:** `07417d9c85799949cd3b173067795905513af173` (`Fin_FeedSat_1-stage-transition-v1.1-validated-2026-09-04`)  
...
| `Fin_FeedSat_1_VOLUME` | **Does not exist.** SUPERSEDED PRE-IMPLEMENTATION PROPOSAL |
| When Volume is present | Whenever Adaptive Host exists (`FIN_FEEDSAT_MODEL=adaptive`) |
| Price configuration | Independent `FIN_FEEDSAT_PRICING` |
...
There is no Fin_FeedSat_1 Volume Policy service, layer, or process. Historical APTF “policy” names remain provenance only.
...
Do **not** expose these as `Fin_FeedSat_1_*` environment variables.

### Operational configuration (CURRENT, Phase G)

Inspected conventions: `FIN_FEEDSAT_MODEL=off|adaptive`, `FIN_FEEDSAT_PRICING=off|expm`, unknown values fail startup, pricing requires adaptive.

| Variable | Current meaning |
|---|---|
| `FIN_FEEDSAT_MODEL` | `adaptive` constructs Adaptive Host. Volume engines are allocated with that Host. `off` → no Host → Volume not wired. |
| `FIN_FEEDSAT_PRICING` | Independent Price opt-in (`off` / `expm`). **Not** required for Volume. |
| `FIN_FEEDSAT_MODEL_DEADLINE` | Shared worker deadline; Volume prepare is in the same handle, not a second mailbox. |
| `Fin_FeedSat_1_VOLUME` | **Does not exist.** |

Volume is on whenever Adaptive Host exists. There is no Volume-only env flag.

#### SUPERSEDED PRE-IMPLEMENTATION PROPOSAL — `Fin_FeedSat_1_VOLUME`

The 2026-09-05 implementation design proposed `Fin_FeedSat_1_VOLUME=off|on` (unknown fail startup; `on` requires `adaptive`). Phase G did **not** implement that flag. Do not treat the old table as operator guidance.

## Assumptions

- Phase 1 Volume design and Phase 2 Process Model win over historical APTF runtime/batch mechanics.
- Inspected host `PrepareStep`/`Commit` and keyed-worker ownership are the Fin_FeedSat_1 realtime precedent.
...
- APTF frozen artifacts are equivalence authorities, not Fin_FeedSat_1 runtime inputs. They are **not** currently vendored in this repository.
...
- Adding a `Fin_FeedSat_1_VOLUME` flag (proposal SUPERSEDED)
...
|---|---|---|
| 1 | [Fin_FeedSat_1_VOLUME_ENGINE_090526.md](../design/Fin_FeedSat_1_VOLUME_ENGINE_090526.md) | Frozen Volume mathematics, lifecycle, maturation vs INVALID, ontology |
| 2 | [Fin_FeedSat_1_PROCESS_MODEL_V1_082926.md](../design/Fin_FeedSat_1_PROCESS_MODEL_V1_082926.md) | Process identity P-04V, sibling topology, same accepted Bar. File contents not reconciled in this pass. |
| 3 | Inspected Fin_FeedSat_1 Go (`internal/modelhost`, `internal/pricing`, `internal/adaptive`, `internal/domain`, `internal/config`) | Host join, prepare/commit, keyed ownership, config style |
| 4 | Frozen APTF artifacts + [investigation](../investigations/Fin_FeedSat_1_APTF_VOLUME_ENGINE_GO_REFACTORABILITY_INVESTIGATION_2026-09-05.md) | Equivalence oracles and historical observe semantics |
| 5 | P-03/P-04 design and implementation documents | Precedents only; do not copy Price/Adaptive science |

Where APTF batch/session/replay mechanics differ from Fin_FeedSat_1 realtime architecture: **preserve the Volume mathematics** and **use Fin_FeedSat_1 realtime consume/state/prepare/commit/emit**.
...
**Not created (and not current work):** `internal/config` `VolumeMode` / `Fin_FeedSat_1_VOLUME`.
...
Implemented `domain.VolumeEvent` (internal) plus proto `finfeedsat.v1.VolumeEvent`. **HISTORICAL:** this table originally said “no proto.” SUPERSEDED.
...
Full frozen APTF corpora remain externally hash-verified in the APTF repository (`pre08242026_docs/`). They are not vendored here (86MB+). Full-corpus tests read them only when `Fin_FeedSat_1_P04V_FROZEN_DIR` is set.
...
Local CSV Fin_FeedSat_1 server run (no provider credentials): `Fin_FeedSat_1_SOURCE=csv`, `FIN_FEEDSAT_MODEL=adaptive`, **`FIN_FEEDSAT_PRICING=off`**. Observed Volume startup and MATURING lines on the same process as Adaptive `model skip` / `model decision`. That run overflowed the inherited worker inbox (`STATE_DISCONTINUOUS`) before AVAILABLE. It was **not** a live-market run and did **not** observe Price `pricing emit`.
...
`Fin_FeedSat_1_VOLUME=off` was never implemented. Volume is present whenever Adaptive Host exists. `FIN_FEEDSAT_MODEL=off` → no Host → Volume not wired.
...
CPU: one small SVD plus tiny sorts per published bar per entity. Memory: a few hundred bytes per entity plus the Engine struct. Latency: Volume runs in the same keyed-worker handle (sequential with Adaptive/Price, not a parallel goroutine). It must fit in `FIN_FEEDSAT_MODEL_DEADLINE` without adding a second mailbox. Physical concurrency is not required for scientific independence.

## 21. Observability

Observational only. Not a second science path. There is **no** independent Volume off mode and **no** `Fin_FeedSat_1_VOLUME` switch.
...
- Adaptive Host absent (`FIN_FEEDSAT_MODEL=off`) → Volume not wired (`StreamVolumeEvents` `FailedPrecondition`)
...
| **G** | Host join + isolation + `ResetSymbol` + delivery tests | **IMPLEMENTED** 2026-09-06. Plan’s `Fin_FeedSat_1_VOLUME` flag was **not** used. |
...
- 014C interpretation corpus is session-reset in APTF; Fin_FeedSat_1 V1 is not. Harness must slice or inject test resets.
...
**DEFERRED / NOT YET AUTHORIZED:** a controlled Fin_FeedSat_1-wide `cockpit` → `Indicator` audit across P-04 Price, StageTransition, Process Model, and dashboard. Do not treat that as part of P-04V V1.
...
**Validation document:** [Fin_FeedSat_1_P04V_VOLUME_FROZEN_EQUIVALENCE_VALIDATION_2026-09-05.md](../investigations/Fin_FeedSat_1_P04V_VOLUME_FROZEN_EQUIVALENCE_VALIDATION_2026-09-05.md)
...
**Purpose:** Add the already-validated P-04V Volume Engine to the canonical Fin_FeedSat_1 protobuf contract. Science is unchanged. **HISTORICAL (contract phase):** ModelHost was not yet wired. Phase G (2026-09-06) wired `StreamVolumeEvents` to Host-produced events.

**Source proto path:** `api/proto/Fin_FeedSat_1/v1/Fin_FeedSat_1.proto`  
**Package / go_package:** `finfeedsat.v1` / `Fin_FeedSat_1/gen/Fin_FeedSat_1/v1;finfeedsatv1`
...
**Generated artifacts:** `gen/Fin_FeedSat_1/v1/Fin_FeedSat_1.pb.go`, `gen/Fin_FeedSat_1/v1/Fin_FeedSat_1_grpc.pb.go` via `buf generate`. Not manually edited.
...
**Configuration:** Volume is on whenever Adaptive Host exists. No `Fin_FeedSat_1_VOLUME` flag. Price remains `FIN_FEEDSAT_PRICING`. Logging uses the standard library logger with no extra env (same as `pricing emit` / `model skip`).
...
**Known limitations:** StageTransition still P-03/P-04 only. Semantic catalog still lacks P-04V terms. Dashboard proto copy not updated. Volume is Adaptive-host-scoped (Host is nil when `FIN_FEEDSAT_MODEL=off`).
...
| 2026-09-06 | Documentation reconciliation: renamed this file to `Fin_FeedSat_1_P04V_VOLUME_ENGINE_IMPLEMENTATION_090526.md`; header/status reconciled; Current Validated Implementation added; stale no-Go/no-proto/no-host claims marked historical; `Fin_FeedSat_1_VOLUME` marked SUPERSEDED PRE-IMPLEMENTATION PROPOSAL; Phase F FAIL preserved as history and F-R PASS stated as current; Phase G marked implemented; P-04V Indicator terminology aligned (historical `cockpit_color` retained as provenance). No science/code/proto/Process Model changes in this documentation pass. |
