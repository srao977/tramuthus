...
- **Finalized bar:** Fin_FeedSat_1 has passed its allowed-lateness threshold and declares the bar stable enough for inference.
...
- **Mid-bar failover:** Fin_FeedSat_1 switches from Alpaca to Databento before the current interval is finalized.
...
2. **Locally aggregated bars:** Fin_FeedSat_1 consumes individual trades and calculates OHLCV itself.

Fin_FeedSat_1 needs to decide which form is authoritative for each interval. Mixing a partial locally calculated bar with a provider-supplied replacement without reconciliation can distort high, low, close, and especially volume.
...
Published indices also require special treatment. Their open, high, low, and close may be useful, but volume may be unavailable or provider-defined. Fin_FeedSat_1 should represent unavailable index volume as nullable or explicitly unsupported, not automatically as zero.
